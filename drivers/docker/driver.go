...
func (d *Driver) RecoverTask(handle *drivers.TaskHandle) error {
	...
	dlogger, dloggerPluginClient, err := d.reattachToDockerLogger(handleState.ReattachConfig)
	if err != nil {
		d.logger.Warn("failed to reattach to docker logger process", "error", err)

		dlogger, dloggerPluginClient, err = d.setupNewDockerLogger(container, handle.Config, time.Now())
		if err != nil {
